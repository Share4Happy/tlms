...
import { TaskScope, TaskType, TaskDifficulty } from '@/types/task';
...
export default function CreateTaskPage() {
  ...
  useEffect(() => {
    // Load users for private task assignment
    loadUsers();
  }, []);
  ...
  const loadUsers = async () => {
    try {
      ...
      // Fail silently if not admin, or just empty list
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      alert(error.response?.data?.detail || 'Tạo task thất bại');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-fuchsia-50 py-8">
      <div className="max-w-3xl mx-auto px-4">
        <div className="mb-6 flex items-center justify-between">
            <h1 className="text-2xl font-bold bg-gradient-to-r from-violet-600 to-fuchsia-600 bg-clip-text text-transparent">Tạo Nhiệm vụ Mới</h1>
            <Link href="/mentor" className="text-gray-600 hover:text-violet-600 transition-colors">
                Cancel
            </Link>
        </div>
